...
public class BoundaryStudente extends JFrame {
	...
	public BoundaryStudente() {
		...
		btnSoluzione.setBounds(163, 117, 202, 55);
		...
		btnListaTask.setBounds(163, 216, 202, 55);
		...
		String[] colonne= {"Nome","Cognome","Email","Ruolo","Media"};
		...
		JButton btnClassifica = new JButton("Visualizza La classifica della classe per punteggio medio");
		btnClassifica.addMouseListener(new MouseAdapter() {
		    @Override
		    public void mouseClicked(MouseEvent e) {
		        Object[][] dati = null;
		        try {
		            ArrayList<UtenteDTO> lista_studenti_ordinata = controller_s.getClassificaStudenti();
		            dati = new Object[lista_studenti_ordinata.size()][5];
		            for (int i = 0; i < lista_studenti_ordinata.size(); i++) {
		                dati[i][0] = lista_studenti_ordinata.get(i).getNome(); 
		                dati[i][1] = lista_studenti_ordinata.get(i).getCognome(); 
		                dati[i][2] = lista_studenti_ordinata.get(i).getEmail(); 
		                dati[i][3] = lista_studenti_ordinata.get(i).getRuolo();
		                dati[i][4] = lista_studenti_ordinata.get(i).getMediaPunti();
				        ...
				        JButton btnHome = new JButton("Torna alla home");
				        btnHome.setPreferredSize(new Dimension(150, 30)); // riduce dimensione
		...
		btnClassifica.setBounds(110, 317, 311, 55);
